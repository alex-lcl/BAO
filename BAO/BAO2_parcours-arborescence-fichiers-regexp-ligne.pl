...
Votre Nom : Alexandra LI COMBEAU LONGUET
 usage : perl BAO2_parcours-arborescence-fichiers-regexp-ligne.pl repertoire-a-parcourir rubrique
...
open(my $OUTUDPIPE,">:encoding(utf8)","sortieudpipe-slurp_$rubrique.txt");
...
sub preetiquetage {
    ...
    my $description = $_[1];
	#-----------------etiquetage titre-----------------------------
	open (my $ETI, ">:encoding(utf8)", "temporaire.txt");
    print $ETI $titre;
    close $ETI;
    system ("perl -f ./distrib-treetagger/tokenise-utf8.pl temporaire.txt > test.txt.pos");
    open (my $TEMP, "<:encoding(utf8)", "test.txt.pos");
    $/=undef;
...
#----------------------------------------------
sub etiquetageTT {
    system ("perl -f ./distrib-treetagger/tokenise-utf8.pl sortiexml-slurp_$rubrique.xml | ./distrib-treetagger/tree-tagger.exe ./distrib-treetagger/french-utf8.par -token -lemma -no-unknown -sgml > sortiexml-slurp_TT_$rubrique.txt");
...
sub etiquetageUD {
	...
	system("./distrib-udpipe-1.2.0-bin/udpipe2xml-version-sans-titrevsdescription-v2.pl sortieudpipe-slurp_$rubrique.txt");
}
